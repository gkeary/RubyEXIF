...
class Photo
  ...
  attr_accessor  :original_date_time, :full_path, :exif
  @@dupe_hash = {}   # :key => :dupe_count}
  
    def initialize (exif, stamp = Date.new, full_path= File.new)
      @original_date_time = stamp
      if @@dupe_hash.has_key? stamp
        @@dupe_hash[stamp] += 1
      else
        @@dupe_hash[stamp] = 1
      end
      @full_path= full_path
      @exif = exif
      PhotoCollection::add_to_array self
    end

    def is_dupe?
     return @@dupe_hash[@original_date_time] > 2
    end
    def self.get_hash
      return @@dupe_hash
    end
...
class PhotoCollection
  ...
  def self.add_to_array (ph)
    if ph.class != Photo
      throw "Parameter must be a Photo"
    end
    @@photos.push(ph)
  end
  ...
  def get_first_width
      return EXIFR::JPEG.new(@jpgfiles[0]).width
  end
  ...
  def load_hash
      ...
      @jpgfiles.each do |file|
            ...
            hash = { ex.date_time=> ex, :num => 1, :fullpath=> file }
            if( photo_hash.has_key? ex.date_time )
              b = photo_hash[ex.date_time]
              hash[:num] += 1
              hash[:fullpath] = file
              photo_hash[ex.date_time]= b.push(hash)
            else
              photo_hash[ex.date_time] =  b.push(hash)
            end
      end
  ...
  def get_array_of_photos
    ...
    @jpgfiles.each do |file|
      ex = EXIFR::JPEG.new(file)
      photo = Photo.new(ex, ex.date_time, file)
...
first_pass = populate_hash(e.jpgfiles)
